...
Eight wood types are covered on this line (oak, acacia, birch, jungle, spruce, dark oak,
...
- **The matrix may differ per line — but only in `Matrix.java`.** Since #88, the wood set is
  no longer uniform: pale oak and cherry exist on this line (`v1.21.11`) and on `main`, but
  not on `v1.21.1`, because pale oak doesn't exist in that version's vanilla jar. What may
  differ between lines is exactly the *contents* of `Matrix.WOODS` (and, in principle,
  `Matrix.METALS`) — nothing else.
  ...
  - What may **not** differ: the shape of an arm. Every wood present in this line's
...
| 2026-09-18 | 1.5 | Cherry and pale oak land on this line (#88), 6x3 -> 8x3. States the per-line matrix rule: the wood set may differ between version lines, but only in `Matrix.java` — every other piece of an arm is still all-or-nothing, and porting the matrix down to a line missing a wood is a subtraction, not a rewrite. |
