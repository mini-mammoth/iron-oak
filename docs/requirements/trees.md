...
- [ ] All combinations in `Matrix.arms()` (24 on `v26.2`) grow, and each yields logs of its
...
`./gradlew runDatagen` is a clean no-op on `v26.2`.
...
- [ ] All loot tables exist (24 logs, 24 saplings on `v26.2`) and each drops its own block
