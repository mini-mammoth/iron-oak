...
# Matrix Requirements — 8 woods × 3 metals on `v26.2`
...
Metals: iron, copper, gold. → **24 combinations** on `v26.2`, and per combination two blocks
...
[`AGENTS.md`](../../AGENTS.md). The counts below are `v26.2`'s.

Counts as registered on `v26.2` (`1.3.0+26.2`): 49 blocks (24 logs + 24 saplings + fire bowl),
...
metal (on `v26.2`; a line with a smaller wood set, per `Matrix.java`, lists that line's set).

This tag is what makes one burning recipe per metal sufficient instead of twenty-four.

**Acceptance criteria** (verify: `inspect`, `test`)
- [ ] Each of the three block tags lists exactly eight logs (on `v26.2`)
...
- [ ] Eight block tag files (on `v26.2`), each listing three infused logs, `"replace": false`
...
- **MAT-Q1** ~~Should MAT-01's counts be enforced mechanically?~~ **Answered: yes (#40).**
  ...
  asserts there are exactly as many generators as `Matrix.arms()` (24 on `v26.2`),
