...
public class SeparatorEntity extends BlockEntity implements ImplementedInventory {
    ...
    private int cookingTotalTime = WashingRecipe.DEFAULT_COOKING_TOTAL_TIME;

    public SeparatorEntity(BlockPos pos, BlockState state) {
        super(ModEntityTypes.SEPARATOR_ENTITY, pos, state);
    }
    ...
    public static void clientTick(World world, BlockPos pos, BlockState state, SeparatorEntity dryRack) {

    }

    public static void serverTick(World world, BlockPos pos, BlockState state, SeparatorEntity dryRack) {
    ...
    public Optional<WashingRecipe> getRecipeFor(ItemStack item) {
        return !getInput().isEmpty() ? Optional.empty() : this.world.getRecipeManager().getFirstMatch(ModRecipes.WASHING_RECIPE_TYPE, new SimpleInventory(item), this.world);
    }
